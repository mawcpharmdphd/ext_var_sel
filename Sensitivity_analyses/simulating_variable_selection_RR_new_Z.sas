LIBNAME results "C:\Users\michael.webster\Documents\Variable_selection_external_validity\Simulation_results_new_Z";
...
DATA fulldata;
	...
	DO i=1 to 10000;
		...
		IF Z6=1 THEN Z7=RAND('BERNOULLI',0.8);
		ELSE IF Z6=0 THEN Z7=RAND('BERNOULLI',0.2);

		X=RAND('BERNOULLI',0.5);

		logPY=log(0.90) + log(0.67)*X + log(0.67)*Z2 + log(0.67)*Z3 + log(0.67)*Z5 + log(0.67)*Z6 + log(0.67)*Z7 + log(0.67)*X*Z3 + log(0.67)*X*Z6;
		pY=exp(logPY);
		Y=RAND('BERNOULLI',PY);
	...
	DO i=1 to 10000;
		...
		IF Z6=1 THEN Z7=RAND('BERNOULLI',0.6);
		ELSE IF Z6=0 THEN Z7=RAND('BERNOULLI',0.3);
...
PROC GENMOD DATA=predvals12 DESCENDING;
	BY replicate;
	MODEL intrial = Z6 Z7 Z6*Z7 / link=identity dist=binomial;
	OUTPUT OUT=predvals13 pred=predZ6Z7;
RUN;

PROC GENMOD DATA=predvals13 DESCENDING;
	BY replicate;
	MODEL intrial = Z1 Z6 Z7 Z6*Z7 / link=identity dist=binomial;
	OUTPUT OUT=predvals14 pred=predZ1Z6Z7;
RUN;

PROC GENMOD DATA=predvals14 DESCENDING;
	BY replicate;
	MODEL intrial = Z2 Z6 Z7 Z6*Z7 / link=identity dist=binomial;
	OUTPUT OUT=predvals15 pred=predZ2Z6Z7;
RUN;

PROC GENMOD DATA=predvals15 DESCENDING;
	BY replicate;
	MODEL intrial = Z3 Z6 Z7 Z6*Z7 / link=identity dist=binomial;
	OUTPUT OUT=predvals16 pred=predZ3Z6Z7;
RUN;

PROC GENMOD DATA=predvals16 DESCENDING;
	BY replicate;
	MODEL intrial = Z4 Z6 Z7 Z6*Z7 / link=identity dist=binomial;
	OUTPUT OUT=predvals17 pred=predZ4Z6Z7;
RUN;

PROC GENMOD DATA=predvals17 DESCENDING;
	BY replicate;
	MODEL intrial = Z5 Z6 Z7 Z6*Z7 / link=identity dist=binomial;
	OUTPUT OUT=predvals18 pred=predZ5Z6Z7;
RUN;
...
PROC GENMOD DATA=makeweights DESCENDING;
	...
	MODEL Y=X / link=log dist=binomial;
	WEIGHT IOSWZ1Z6Z7;
	ODS OUTPUT parameterestimates=ioswZ1Z6Z7;
RUN;
...
PROC GENMOD DATA=makeweights DESCENDING;
	...
	MODEL Y=X / link=log dist=binomial;
	WEIGHT IOSWZ2Z6Z7;
	ODS OUTPUT parameterestimates=ioswZ2Z6Z7;
RUN;
...
PROC GENMOD DATA=makeweights DESCENDING;
	...
	MODEL Y=X / link=log dist=binomial;
	WEIGHT IOSWZ3Z6Z7;
	ODS OUTPUT parameterestimates=ioswZ3Z6Z7;
RUN;
...
PROC GENMOD DATA=makeweights DESCENDING;
	...
	MODEL Y=X / link=log dist=binomial;
	WEIGHT IOSWZ4Z6Z7;
	ODS OUTPUT parameterestimates=ioswZ4Z6Z7;
RUN;
...
PROC GENMOD DATA=makeweights DESCENDING;
	...
	MODEL Y=X / link=log dist=binomial;
	WEIGHT IOSWZ5Z6Z7;
	ODS OUTPUT parameterestimates=ioswZ5Z6Z7;
RUN;
...
PROC DATASETS library = results nolist;
	append base=results.ioswZ1Z6Z7 data=work.ioswZ1Z6Z7;
QUIT;

PROC DATASETS library = results nolist;
	append base=results.ioswZ2Z6Z7 data=work.ioswZ2Z6Z7;
QUIT;

PROC DATASETS library = results nolist;
	append base=results.ioswZ3Z6Z7 data=work.ioswZ3Z6Z7;
QUIT;

PROC DATASETS library = results nolist;
	append base=results.ioswZ4Z6Z7 data=work.ioswZ4Z6Z7;
QUIT;

PROC DATASETS library = results nolist;
	append base=results.ioswZ5Z6Z7 data=work.ioswZ5Z6Z7;
QUIT;
...
%simulate(repstart=11,repend=20000,seed=9992233);
